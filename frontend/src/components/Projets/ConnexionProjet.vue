        ...
        <form class="review-form" enctype="multipart/form-data">
            <div class=" information">
                ...
                <div class="titre-input">
                    ...
                    <input type="email" name="mail" class="toto" required v-model="email" placeholder="you@gmail.com"
                        pattern="(^[a-z0-9]+)@([a-z0-9])+(\.)([a-z]{2,4})">
                    <span v-if="msg.email" class="vert">{{ msg.email }} </span>
                </div>
                <div class="titre-input">
                    <p class="titre"><i> Mot de passe:</i></p>
                    <input type="password" class="toto" v-model="password" required name="mot de passe">
                    <span v-if="msg.password" class="vert">{{ msg.password }} </span>
                    <br>
                    <router-link :to="lien" class="none">
                        <div class="bou" v-on:click="show('Veuillez entrez un email ou un mot de passe valide')">
                            <p>Se connecter</p>
                        </div>
                    </router-link>
...
export default {
    ...
    methods: {
        ...
        validatePassword(value) {
            ...
            if (value.length < 10) {
                this.msg['password'] = 'Sa doit contenir au moins dix caractère!' + difference + ' caractères restantes';
            } else {
                this.msg['password'] = '';
            }
        },
